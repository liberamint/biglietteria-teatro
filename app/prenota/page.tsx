...
import { Container, PageShell, Card, CardHeader, CardTitle, CardContent, Input, Textarea, Button } from '@/components/ui';
...
export default function PrenotaPage() {
  ...
  return (
    <PageShell>
      <Container>
        <Card>
          <CardHeader><CardTitle>Prenotazione biglietti</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium">Spettacolo</label>
                <select className="mt-1 w-full rounded-2xl border bg-white px-4 py-3 text-sm" value={showSlug} onChange={(e) => setShowSlug(e.target.value)}>
                  {SHOWS.map((show) => (
...
<div>
  ...
  />
              ...
              <Button onClick={submitBooking} disabled={busy || !requesterName || !phone} className="w-full">
                {busy ? 'Invio...' : 'Invia richiesta'}
              </Button>
              <p className="text-sm text-zinc-600">
                Per ogni nominativo farà fede l&apos;ultima richiesta inviata. La conferma del pagamento e i seriali dei biglietti vengono inseriti solo dall&apos;organizzazione.
              </p>
